...
- **Backend hosting**: **live on Render** at `https://farmpay-1l94.onrender.com` (not Fly.io/Koyeb — both were ruled out: Fly.io needs a credit card, Koyeb was mid-acquisition by Mistral AI with a broken-looking dashboard at the time this was evaluated). Deployed as a native Node web service (Render auto-detected Node and runs `npm install` / `node backend/src/app.js` directly — no Docker involved on this host, despite `deploy/Dockerfile` existing in the repo for other environments). Confirmed responding and connected to Supabase. Free tier sleeps after 15 min idle (30-50s cold start on next request) — acceptable for pilot, revisit for production. **Point the Flutter app's API base URL at this URL.**
